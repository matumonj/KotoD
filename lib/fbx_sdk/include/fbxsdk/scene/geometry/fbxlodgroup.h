...
/** Defines a LOD (Level of Detail) group.
  ...
  * transform are controlled by the m_Distance of a group to the camera and the
  * threshold values.
  *
...
public:
	...
	/** Specifies if the threshold values stored by this LOD object are defining
	  * a m_Distance to the camera (by default) or a percentage value.
	  *
	...
	//@{
		/** This property handles the use of the Min/Max distances.
		  * Enables the minimum and maximum m_Distance to take effect.
		  * For example, if the m_Distance between the group and the camera is smaller 
		  * than the minimum m_Distance, then the whole group disappears.
		  *
		...
		/** The minimum m_Distance at which the group is displayed.
		  *
		...
		/** The maximum m_Distance at which the group is displayed. 
		  *
		...
		/** Work in world space of transform or local space If true, 
		  * the camera m_Distance to the LOD group will be computed in world space.
		  ...
		  * the m_Distance computation ignores any parent transforms of the LOD transform. 
		  *
	...
	/** Add a new threshold. 
	  * \param pThreshValue Threshold value (m_Distance).
	...
	/** Replace the value of the specified threshold.
	  ...
	  * \param pThreshValue New threshold value (m_Distance).
	  * \return true if successful and false otherwise.
	  * \remarks This method will fail if ThresholdsUsedAsPercentage=true.
	  */
	...
	/** Add a new displayLevel value to the current list.
	  ...
	  * or show the object at that level. For example, if the m_Distance between
