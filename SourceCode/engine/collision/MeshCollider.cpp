...
void MeshCollider::ConstructTriangles(IKEModel* model)
{
	triangles.clear();
	
	const std::vector<IKEMesh*>& meshes = model->GetMeshes();

	int start = 0;

	std::vector<IKEMesh*>::const_iterator it = meshes.cbegin();
	for (; it != meshes.cend(); ++it) {
		IKEMesh* mesh = *it;
		const std::vector<IKEMesh::VertexPosNormalUv>& vertices = mesh->GetVertices();
...
bool MeshCollider::CheckCollisionRay(const Ray & ray, float * m_Distance, DirectX::XMVECTOR * inter)
{
	...
	for (; it != triangles.cend(); ++it) {
		...
		if (Collision::CheckRay2Triangle(localRay, triangle, nullptr, &tempInter)) {
			...
			if (m_Distance) {
				...
				*m_Distance = XMVector3Dot(sub, ray.dir).m128_f32[0];
			}
