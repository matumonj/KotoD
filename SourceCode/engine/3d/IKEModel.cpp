...
/// 静的メンバ変数の実体
const std::string IKEModel::baseDirectory = "Resources/3d/";
...
void IKEModel::Initialize(const std::string& modelname, bool smoothing)
{
	// モデル読み込み
...
void IKEModel::LoadModel(const std::string& modelname, bool smoothing)
{
	...
	// メッシュ生成
	meshes.emplace_back(new IKEMesh);
	IKEMesh* mesh = meshes.back();
	int indexCountTex = 0;
...
void IKEModel::LoadMaterial(const std::string& directoryPath, const std::string& filename)
{
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
...
void IKEModel::AddMaterial(IKEMaterial* material)
{
	// コンテナに登録
	materials.emplace(material->name, material);
}

void IKEModel::CreateDescriptorHeap()
{
