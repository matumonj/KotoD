...
namespace ImGui
{
    ...
    IMGUI_API float         GetTextLineHeightWithSpacing();                                 // ~ FontSize + style.ItemSpacing.y (m_Distance in pixels between 2 consecutive lines of text)
    IMGUI_API float         GetFrameHeight();                                               // ~ FontSize + style.FramePadding.y * 2
    IMGUI_API float         GetFrameHeightWithSpacing();                                    // ~ FontSize + style.FramePadding.y * 2 + style.ItemSpacing.y (m_Distance in pixels between 2 consecutive lines of framed widgets)
    ...
    IMGUI_API void          Bullet();                                                       // draw a small circle + keep the cursor on the same line. advance cursor x position by GetTreeNodeToLabelSpacing(), same m_Distance that TreeNode() uses
    ...
    IMGUI_API float         GetTreeNodeToLabelSpacing();                                        // horizontal m_Distance preceding label when using TreeNode*() or Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular unframed TreeNode
    ...
    // - Dragging operations are only reported after mouse has moved a certain m_Distance away from the initial clicking position (see 'lock_threshold' and 'io.MouseDraggingThreshold')
    ...
    IMGUI_API ImVec2        GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);   // return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a m_Distance threshold at least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
...
struct ImGuiIO
{
    ...
    float       MouseDoubleClickMaxDist;        // = 6.0f           // m_Distance threshold to stay in to validate a double-click, in pixels.
    float       MouseDragThreshold;             // = 6.0f           // m_Distance threshold before considering we are dragging.
    ...
    ImVec2      MouseDragMaxDistanceAbs[5];     // Maximum m_Distance, absolute, on each axis, of how much mouse has traveled from the clicking point
    float       MouseDragMaxDistanceSqr[5];     // Squared maximum m_Distance of how much mouse has traveled from the clicking point
...
struct ImGuiListClipper
{
    ...
    // items_height: Use -1.0f to be calculated automatically on first step. Otherwise pass in the m_Distance between your items, typically GetTextLineHeightWithSpacing() or GetFrameHeightWithSpacing().
...
struct ImFontGlyph
{
    ...
    float           AdvanceX;           // m_Distance to next character (= data from font + ImFontConfig::GlyphExtraSpacing.x baked in)
...
struct ImFont
{
    ...
    float                       Ascent, Descent;    // 4+4   // out //            // Ascent: m_Distance from top to bottom of e.g. 'A' [0..FontSize]
